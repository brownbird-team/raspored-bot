...
export const createEmailLoginModal = () => {
    if (document.querySelector(".email-login-root") != null) {
        return;
    }

    const root = document.createElement("div");
    root.classList.add("modal-root");
    root.classList.add("email-login-root");
    ...
    registrationButtonName.innerText = "Prijavi se";
    ...
    registrationButton.addEventListener("click", async() => {
        ...
        await fetch("/api/email/auth/login", {
            headers: {
            ...
            if (json.status == "error") {
                switch(json.name) {
                    case "TooManyAttemptsError":
                        createNotification("Kreirali ste previše tokena za prijavu, pokušajte ponovno kasnije", false);
                        break;
                    case "NotFoundError":
                        createNotification("Korisnički račun ne postoji", false);
                        break;
                    case "EmailError": 
